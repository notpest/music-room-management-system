...
const SlotTable = () => {
  ...
  // Render the Calendar component in a modal
  const renderCalendarModal = () => (
    <Modal isOpen={weekPickerOpen} onOpenChange={setWeekPickerOpen}>
      <ModalContent>
        ...
        <ModalBody>
          <Calendar
            aria-label="Select a date to view the week"
            value={selectedDate}
            onChange={(date) => {
              setSelectedDate(date);
              setWeekPickerOpen(false); // Close the modal after selecting a date
            }}
          />
        </ModalBody>
        <ModalFooter>
      ...
      <div style={{ marginBottom: "1rem" }}>
        ...
        {isCalendarOpen && (
          <Calendar
            ...
            onChange={(e) => setTempWeekDate(e)} // Only update tempDate, not selectedDate yet
          />
        )}
      </div>
    </ModalBody>
